...
class BlogsControllerTest {
	...
	@Test
	@DisplayName("Unit-Test Get All Blogs")
	void testGetAllBlogs() throws Exception {
		...
		mockMvc.perform(get("/blogs")).andExpect(status().isOk()).andExpect(jsonPath("$.length()").value(2))
				.andExpect(jsonPath("$[0].title").value("Blog 1")).andExpect(jsonPath("$[1].title").value("Blog 2"));

		verify(blogservice, times(1)).getAllBlogs();
	}

	@Test
	@DisplayName("Unit-Test Get Blogs By id - success")
	void testGetBlogById_Success() throws Exception {
		...
		mockMvc.perform(get("/blogs/1")).andExpect(status().isOk()).andExpect(jsonPath("$.title").value("Blog 1"))
	...
	@Test
	@DisplayName("Unit-Test Get Blogs By missing id - not found")
	void testGetBlogById_NotFound() throws Exception {
		...
		mockMvc.perform(get("/blogs/1")).andExpect(status().isNotFound()).andExpect(content().string("Blog not found"));
	...
	void testCreateBlog() throws Exception {
		...
		mockMvc.perform(post("/blogs/post").contentType(MediaType.APPLICATION_JSON).content(json))
				.andExpect(status().isCreated()).andExpect(jsonPath("$.title").value("Blog 1"));

		verify(blogservice, times(1)).createBlogs(any(Blogs.class));
	}

	@Test
	@DisplayName("Unit-Test Update Blog")
	void testUpdateBlog() throws Exception {
		...
		mockMvc.perform(put("/blogs/update/1").contentType(MediaType.APPLICATION_JSON).content(json))
	...
	@Test
	@DisplayName("Unit-Test Delete Blog By Id")
	void testDeleteBlogById() throws Exception {
		doNothing().when(blogservice).deleteById(1L);

		mockMvc.perform(delete("/blogs/delete/1")).andExpect(status().isOk());
	...
	void testDeleteAllBlogs() throws Exception {
		...
		mockMvc.perform(delete("/blogs/delete")).andExpect(status().isOk());
